...
class EasyMessagePack(val packer: MessagePacker = MessagePack.newDefaultBufferPacker()) {
    ...
    fun unpack(inputStream: InputStream): EasyMessagePack {
        unpacker = MessagePack.newDefaultUnpacker(inputStream)
        return this
    }

    fun putBoolean(boolean: Boolean?): EasyMessagePack {
        if (boolean == null) {
            packer.packNil()
        } else {
    ...
    fun putBooleanArray(booleanArray: BooleanArray?): EasyMessagePack {
    ...
    fun putBooleanList(booleanList: List<Boolean?>?): EasyMessagePack {
        if (booleanList == null) {
            packer.packNil()
        } else {
    ...
    fun putByte(byte: Byte?): EasyMessagePack {
        if (byte == null) {
            packer.packNil()
        } else {
    ...
    fun putByteArray(byteArray: ByteArray?): EasyMessagePack {
    ...
    fun putByteList(byteList: List<Byte?>?): EasyMessagePack {
        if (byteList == null) {
            packer.packNil()
        } else {
    ...
    fun putShort(short: Short?): EasyMessagePack {
        if (short == null) {
            packer.packNil()
        } else {
    ...
    fun putShortArray(shortArray: ShortArray?): EasyMessagePack {
    ...
    fun putIntArray(intArray: IntArray?): EasyMessagePack {
    ...
    fun putIntList(intList: List<Int?>?): EasyMessagePack {
        if (intList == null) {
            packer.packNil()
        } else {
    ...
    fun putLong(long: Long?): EasyMessagePack {
        if (long == null) {
            packer.packNil()
        } else {
    ...
    fun putLongArray(longArray: LongArray?): EasyMessagePack {
    ...
    fun putLongList(list: List<Long?>?): EasyMessagePack {
        if (list == null) {
            packer.packNil()
        } else {
    ...
    fun putBigInteger(bigInteger: BigInteger?): EasyMessagePack {
        if (bigInteger == null) {
            packer.packNil()
        } else {
    ...
    fun putBigIntegerArray(array: Array<BigInteger?>?): EasyMessagePack {
    ...
    fun putBigIntegerList(list: List<BigInteger?>?): EasyMessagePack {
        if (list == null) {
            packer.packNil()
        } else {
            packer.packArrayHeader(list.size)
    ...
    fun putFloat(float: Float?): EasyMessagePack {
        if (float == null) {
            packer.packNil()
        } else {
    ...
    fun putFloatArray(floatArray: FloatArray?): EasyMessagePack {
    ...
    fun putFloatList(list: List<Float?>?): EasyMessagePack {
        if (list == null) {
            packer.packNil()
        } else {
            packer.packArrayHeader(list.size)
    ...
    fun putDouble(double: Double?): EasyMessagePack {
        if (double == null) {
            packer.packNil()
        } else {
    ...
    fun putDoubleArray(doubleArray: DoubleArray?): EasyMessagePack {
    ...
    fun putDoubleList(list: List<Double?>?): EasyMessagePack {
        if (list == null) {
            packer.packNil()
        } else {
            packer.packArrayHeader(list.size)
    ...
    fun putString(string: String?): EasyMessagePack {
        if (string == null) {
            packer.packNil()
        } else {
    ...
    fun putStringArray(array: Array<String?>?): EasyMessagePack {
        if (array == null || array.isEmpty()) {
            packer.packNil()
        } else {
            packer.packArrayHeader(array.size)
    ...
    fun putStringList(list: List<String?>?): EasyMessagePack {
        if (list == null) {
            packer.packNil()
        } else {
            packer.packArrayHeader(list.size)
    ...
    fun <T, K> putMap(map: Map<T, K>?): EasyMessagePack {
        if (map == null) {
            packer.packNil()
            return this
        }
    ...
    fun putCustomPackData(data: CustomPackData?): EasyMessagePack {
        return if (data == null) {
            packer.packNil()
            this
        } else {
            ...
            data.packTo(this)
            if (oldSize == packer.totalWrittenBytes) throw IllegalStateException("CustomPackData没有提交任何资料给CustomPackData，违反了规定")
            this
        }
    }

    inline fun <reified T : CustomPackData> getCustomPackData(clazz: Class<T>): T? {
        if (unpacker.tryUnpackNil()) return null
    ...
    inline fun <reified T : CustomPackData, reified K> getCustomPackData(clazz: Class<T>, owner: K): T? {
        if (unpacker.tryUnpackNil()) return null
    ...
    fun putArray(array: Array<out CustomPackData?>?): EasyMessagePack {
        if (array == null || array.isEmpty()) {
            packer.packNil()
        } else {
            packer.packArrayHeader(array.size)
            array.forEach {
                putCustomPackData(it)
            }
        }
        return this
    }
    ...
    fun putList(list: List<CustomPackData?>?): EasyMessagePack {
        if (list == null) {
            packer.packNil()
        } else {
            packer.packArrayHeader(list.size)
    ...
    inline fun <reified T : CustomPackData> getList(clazz: Class<T>): ArrayList<T?>? {
        if (unpacker.tryUnpackNil()) return null
        val size = unpacker.unpackArrayHeader()
        val arrayList = ArrayList<T?>()
        for (i in 0 until size) {
            arrayList.add(getCustomPackData(clazz))
        }
        return arrayList
    }

    inline fun <reified T : CustomPackData, reified K> getList(clazz: Class<T>, owner: K): ArrayList<T?>? {
        if (unpacker.tryUnpackNil()) return null
        val size = unpacker.unpackArrayHeader()
        val arrayList = ArrayList<T?>()
        for (i in 0 until size) {
            arrayList.add(getCustomPackData(clazz, owner))
        }
        return arrayList
    }
    ...
    private fun writeAuto(it: Any?) {
        ...
        when (it) {
            ...
            is CustomPackData -> putCustomPackData(it)
            is List<*> -> {
                packer.packArrayHeader(it.size)
                it.forEach {
                    put(it)
            ...
            is Map<*, *> -> {
                ...
                error("EasyMessagePack: error type : ${it.javaClass.name}")
            }
        }
    }
